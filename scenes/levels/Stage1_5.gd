...
const STAGE_ID := "1-5"
const WAVE_COUNTS := [8, 9, 11]
const WAVE_G2_COUNTS := [0, 3, 3]
const WAVE_G3_COUNTS := [3, 2, 3]
const WAVE_TRANSITION_DELAY := 1.2
const SPAWN_VALIDATION_RADIUS := 26.0
const SPAWN_VALIDATION_RETRIES := 6
...
func _ready() -> void:
	...
	_set_hud_status("Stage 1-5 start: Survive the upper and lower pressure")
	_start_next_wave()
...
func _spawn_wave(count: int) -> void:
	...
	for i: int in range(count):
		...
		enemy.global_position = _get_validated_spawn_position(_current_wave_index, i, count)
...
func _get_wave_spawn_position(wave_index: int, enemy_index: int, total_count: int) -> Vector2:
	var g3_count: int = WAVE_G3_COUNTS[wave_index]
	var g2_count: int = WAVE_G2_COUNTS[wave_index]
	var is_g3: bool = enemy_index >= total_count - g3_count
	var is_g2: bool = enemy_index >= total_count - g3_count - g2_count and not is_g3
	match wave_index:
		0:
			if is_g3:
				var vertical_sign: float = -1.0 if enemy_index % 2 == 0 else 1.0
				return player_spawn.global_position + Vector2(
					randf_range(-260.0, 260.0),
					vertical_sign * randf_range(250.0, 330.0)
				)
			return player_spawn.global_position + Vector2(
		...
		1:
			if is_g3:
				var ranged_sign: float = -1.0 if enemy_index % 2 == 0 else 1.0
				return player_spawn.global_position + Vector2(
					randf_range(-300.0, 300.0),
					ranged_sign * randf_range(260.0, 340.0)
				)
			if is_g2:
				var charger_sign: float = -1.0 if enemy_index % 2 == 0 else 1.0
				return player_spawn.global_position + Vector2(
					charger_sign * randf_range(250.0, 420.0),
		...
		2:
			if is_g3:
				var far_vertical_sign: float = -1.0 if enemy_index % 2 == 0 else 1.0
				return player_spawn.global_position + Vector2(
					randf_range(-340.0, 340.0),
					far_vertical_sign * randf_range(270.0, 360.0)
				)
			if is_g2:
				var flank_sign: float = -1.0 if enemy_index % 2 == 0 else 1.0
				return player_spawn.global_position + Vector2(
					flank_sign * randf_range(280.0, 430.0),
					randf_range(-170.0, 170.0)
				)
			return player_spawn.global_position + Vector2(
...
func _get_validated_spawn_position(wave_index: int, enemy_index: int, total_count: int) -> Vector2:
	for _attempt: int in range(SPAWN_VALIDATION_RETRIES):
		var candidate := _get_wave_spawn_position(wave_index, enemy_index, total_count)
		if _is_spawn_position_valid(candidate):
			return candidate
	return _get_fallback_spawn_position(enemy_index)

func _is_spawn_position_valid(position: Vector2) -> bool:
	var shape := CircleShape2D.new()
	shape.radius = SPAWN_VALIDATION_RADIUS
...
func _get_fallback_spawn_position(enemy_index: int) -> Vector2:
	var side_sign: float = -1.0 if enemy_index % 2 == 0 else 1.0
	return player_spawn.global_position + Vector2(
		side_sign * randf_range(90.0, 160.0),
		randf_range(-110.0, 110.0)
	)
...
func _start_next_wave() -> void:
	...
	_set_hud_status("Stage 1-5: Wave %d in progress" % (_current_wave_index + 1))
...
func _spawn_boss() -> void:
	...
	_set_hud_status("Stage 1-5 boss engaged")
...
func _on_enemy_died(_enemy: EnemyBase) -> void:
	...
	if _active_enemy_count == 0 and not _boss_spawned:
		if _current_wave_index + 1 < WAVE_COUNTS.size():
			...
			_set_hud_status("Stage 1-5: Next wave incoming")
...
func _on_boss_died(boss: EnemyBase) -> void:
	...
	_set_hud_status("Stage 1-5 boss defeated. Pick up the equipment")
...
func _on_equipment_equipped(equipment_id: String) -> void:
	...
	if not equipment_id.is_empty():
		...
		_set_hud_status("Equipped: %s. Stage 1-5 clear! Returning to Hub..." % equipped_name)
	else:
		_set_hud_status("Stage 1-5 clear! Returning to Hub...")
...
func _on_player_died() -> void:
	...
	_set_hud_status("You died in Stage 1-5. Returning to Hub...")
	_return_to_hub(2.0)
