...
const STAGE_ID := "1-4"
const WAVE_COUNTS := [8, 9, 10]
const WAVE_G2_COUNTS := [0, 2, 3]
const WAVE_G3_COUNTS := [2, 2, 2]
...
func _ready() -> void:
	...
	_set_hud_status("Stage 1-4 start: Hold the center and clear wave 1")
	_start_next_wave()
...
func _spawn_wave(count: int) -> void:
	...
	for i: int in range(count):
		...
		enemy.global_position = _get_wave_spawn_position(_current_wave_index, i, count)
...
func _get_wave_spawn_position(wave_index: int, enemy_index: int, total_count: int) -> Vector2:
	var is_g3: bool = enemy_index >= total_count - WAVE_G3_COUNTS[wave_index]
	match wave_index:
		0:
			if is_g3:
				var side_sign: float = -1.0 if enemy_index % 2 == 0 else 1.0
				return player_spawn.global_position + Vector2(
					side_sign * randf_range(340.0, 460.0),
					randf_range(-190.0, -40.0)
				)
			return player_spawn.global_position + Vector2(
				randf_range(-220.0, 220.0),
				randf_range(-260.0, -90.0)
			)
		1:
			if is_g3:
				var ranged_side_sign: float = -1.0 if enemy_index % 2 == 0 else 1.0
				return player_spawn.global_position + Vector2(
					ranged_side_sign * randf_range(360.0, 500.0),
					randf_range(-120.0, 120.0)
				)
			var side_sign: float = -1.0 if enemy_index % 2 == 0 else 1.0
			return player_spawn.global_position + Vector2(
				side_sign * randf_range(220.0, 420.0),
				randf_range(-240.0, 200.0)
			)
		2:
			if is_g3:
				var far_side_sign: float = -1.0 if enemy_index % 2 == 0 else 1.0
				return player_spawn.global_position + Vector2(
					far_side_sign * randf_range(380.0, 520.0),
					randf_range(-220.0, 160.0)
				)
			if enemy_index % 3 == 0:
				return player_spawn.global_position + Vector2(
					randf_range(-120.0, 120.0),
					randf_range(-300.0, -180.0)
				)
			return player_spawn.global_position + Vector2(
				(-1.0 if enemy_index % 2 == 0 else 1.0) * randf_range(220.0, 420.0),
				randf_range(-250.0, 230.0)
			)
		_:
			return player_spawn.global_position + Vector2(
...
func _start_next_wave() -> void:
	...
	_set_hud_status("Stage 1-4: Wave %d in progress" % (_current_wave_index + 1))
...
func _spawn_boss() -> void:
	...
	_set_hud_status("Stage 1-4 boss engaged")
...
func _on_enemy_died(_enemy: EnemyBase) -> void:
	...
	if _active_enemy_count == 0 and not _boss_spawned:
		if _current_wave_index + 1 < WAVE_COUNTS.size():
			...
			_set_hud_status("Stage 1-4: Next wave incoming")
...
func _on_boss_died(boss: EnemyBase) -> void:
	...
	_set_hud_status("Stage 1-4 boss defeated. Pick up the equipment")
...
func _on_equipment_equipped(equipment_id: String) -> void:
	...
	if not equipment_id.is_empty():
		...
		_set_hud_status("Equipped: %s. Stage 1-4 clear! Returning to Hub..." % equipped_name)
	else:
		_set_hud_status("Stage 1-4 clear! Returning to Hub...")
...
func _on_player_died() -> void:
	...
	_set_hud_status("You died in Stage 1-4. Returning to Hub...")
	_return_to_hub(2.0)
