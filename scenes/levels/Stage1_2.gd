...
const STAGE_ID := "1-2"
const WAVE_COUNTS := [6, 8]
const SECOND_WAVE_DELAY := 1.2
const SECOND_WAVE_G2_COUNT := 3
...
func _ready() -> void:
	...
	_set_hud_status("Stage 1-2 start: Clear the first wave")
	_start_next_wave()
...
func _get_enemy_scene_for_wave(wave_index: int, enemy_index: int, total_count: int) -> PackedScene:
	if wave_index == 1:
		var g2_start_index: int = max(total_count - SECOND_WAVE_G2_COUNT, 0)
...
func _start_next_wave() -> void:
	...
	_set_hud_status("Stage 1-2: Wave %d in progress" % (_current_wave_index + 1))
...
func _on_enemy_died(_enemy: EnemyBase) -> void:
	...
	if _active_enemy_count == 0 and not _boss_spawned:
		if _current_wave_index + 1 < WAVE_COUNTS.size():
			...
			_set_hud_status("Stage 1-2: Second wave incoming")

			var timer: SceneTreeTimer = get_tree().create_timer(SECOND_WAVE_DELAY)
