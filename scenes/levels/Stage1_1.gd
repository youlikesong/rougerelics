...
func _ready() -> void:
	...
	_set_hud_status("Clear mobs to summon Boss 1-1")
...
func _spawn_boss() -> void:
	...
	_set_hud_status("Boss 1-1 engaged")
...
func _on_boss_died(boss: EnemyBase) -> void:
	...
	if drop_id.is_empty():
		# Misconfigured drop data should not soft-lock stage completion.
		_on_equipment_equipped("")
		return
	var pickup := PICKUP_SCENE.instantiate() as EquipmentPickup
	if pickup == null:
		# Missing pickup scene should not soft-lock stage completion.
	...
	_set_hud_status("Boss defeated. Pick up the equipment")
...
func _on_equipment_equipped(equipment_id: String) -> void:
	...
	if not equipment_id.is_empty():
		...
		_set_hud_status("Equipped: %s. Stage clear! Returning to Hub..." % equipped_name)
	else:
		_set_hud_status("Stage clear! Returning to Hub...")
...
func _on_player_died() -> void:
	if _stage_complete or _is_returning_to_hub:
		return
	_set_hud_status("You died. Returning to Hub...")
	_return_to_hub(2.0)
